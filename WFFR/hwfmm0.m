function res = hwfmm0(Y,model,wpspecs,MCMCspecs,sampleU) 
...
if isempty(model.Z{1})||(sum(sum(model.Z{1})) == 0)
    ...
    [theta_mle,~,theta_sd,~,betans,Vbetans,u_mle,Wv,converge] = mixed_mle(W,theta0,model,Y,wpspecs,MCMCspecs); %#ok<NASGU>   
    initial.u_mle = u_mle;
end

% %% check initial theta values %%
% for t = 1:length(theta_mle)
%     if theta_mle(t) < MCMCspecs.initVarTol
%         theta_mle(t) = theta_mle(t) + MCMCspecs.initVarTol;
%     end
% end
% 
% %% check inital Vbetans
% for i = 1:size(Vbetans,1)
%     for j = 1:size(Vbetans,2)
%         if Vbetans(i,j) < MCMCspecs.initVarTol
%             Vbetans(i,j) = Vbetans(i,j) + MCMCspecs.initVarTol;
%         end
%     end
% end
...
beta            = zeros(size(betans));
Wv.L2           = Get_L2(beta,Wv,wpspecs);              % Update L2 for starting values of beta

%% set up historical coef flag matrix %%
histMark            = ones(wpspecs.V,wpspecs.K);            % needs to be full surface
fullHistCoef        = wavphc(histMark, wpspecsx, wpspecsy); % needs to be full surface, remove thresholded X coef flags
sampHistCoef        = fullHistCoef(model.keep,:);           % model.keep is vector indexing columns of X* to keep
