function res = wpfmm(Y,model,wpspecs,MCMCspecs,sampleU,adhoc)
...
%% if adhoc sampling approach is desired %%
if adhoc
    betanh              = wavphc(beta(1:fsize,:), wpspecsx, wpspecsy);
    beta(1:fsize,:)     = betanh;
end;
...
%% set counter for MCMC samples to output, start timer %%
ii              = 0;
tic
...
for i = 1:(B*thin+burnin)
    ...
    %% (1) update beta %%
    [beta, gamma, alpha]    = UpdateBetaNoOrthog(beta, Vbetans, PiMat, TauMat, Wv, model, wpspecs, MCMCspecs);
    %% if adhoc sampling approach is desired %%
    if adhoc
        betanh              = wavphc(beta(1:fsize,:), wpspecsx, wpspecsy);
        beta(1:fsize,:)     = betanh;
