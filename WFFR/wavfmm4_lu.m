function res = wavfmm4_lu(Y,model,wavespecsy,wavespecsc,pcaspecsx,MCMCspecs,sampleU,get_sigma,twosample,saveraw,blocksize) 
...
if isempty(model.Z{1})||(sum(sum(model.Z{1})) == 0)
    ...
    [theta_mle,~,theta_sd,~,betans,Vbetans,u_mle,Wv,converge] = mixed_mle(W,theta0,model,D,wavespecsy,MCMCspecs); %#ok<NASGU>   
    initial.u_mle = u_mle;
end
...
if sampleU==0        
    postout = PostProcess_lu(MCMC_beta,MCMC_alpha,MCMC_flag_theta,MCMC_tau,MCMC_pi,MCMC_theta,theta,model,wavespecsc,wavespecsy,pcaspecsx,twosample,get_sigma,sampleU);
else
    postout = PostProcess_lu(MCMC_beta,MCMC_alpha,MCMC_flag_theta,MCMC_tau,MCMC_pi,MCMC_theta,theta,model,wavespecsc,wavespecsy,pcaspecsx,twosample,get_sigma,sampleU,MCMC_U);
end
