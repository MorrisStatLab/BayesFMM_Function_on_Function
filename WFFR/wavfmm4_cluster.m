function res = wavfmm4_cluster(Y,model,wavespecsy,wavespecsc,pcaspecsx,MCMCspecs,sampleU,get_sigma,twosample,blocksize,paramroute) 
...
currentblock        = 0;
MCMCspecs.runtime   = zeros((B*thin+burnin)/MCMCspecs.time_update,1);
timecount           = 1;
tic
...
for i = 1:(B*thin+burnin)
    ...
    if mod(i, MCMCspecs.time_update) == 0
       MCMCspecs.runtime(timecount) = toc;
...
%         PostProcess_inMem(MCMC_beta,MCMC_alpha,MCMC_flag_theta,MCMC_theta,MCMC_tau,MCMC_pi,betans,theta,model,wavespecsy,get_sigma,sampleU);
        postout = PostProcess_cluster(MCMC_beta,MCMC_alpha,MCMC_flag_theta,MCMC_tau,MCMC_pi,MCMC_theta,theta,model,wavespecsc,wavespecsy,pcaspecsx,twosample,get_sigma,sampleU);
    else
%         [betahat,beta_025CI,beta_975CI,alphahat,accept_rate_theta,ghat,ghatns,Q025_ghat,Q975_ghat,thetahat,theta_025CI,theta_975CI,tauhat,pihat,...
...
%         PostProcess_inMem(MCMC_beta,MCMC_alpha,MCMC_flag_theta,MCMC_theta,MCMC_tau,MCMC_pi,betans,theta,model,wavespecsy,get_sigma,sampleU,MCMC_U);
        postout = PostProcess_cluster(MCMC_beta,MCMC_alpha,MCMC_flag_theta,MCMC_tau,MCMC_pi,MCMC_theta,theta,model,wavespecsc,wavespecsy,pcaspecsx,twosample,get_sigma,sampleU,MCMC_U);
    end
...
if twosample == 1;
    ...
    res.Q025_bhat2  = postout.Q025_bhat2;
    res.Q975_bhat2  = postout.Q975_bhat2;
    res.bhat2       = postout.bhat2;
