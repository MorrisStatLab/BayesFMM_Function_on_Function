function res = opwavffr(Y,model,wavespecsy,wavespecsx,pcaspecsx,MCMCspecs) 
%OPWAVFFR Performs ordinal probit wavelet based function-on-function 
%           regression. Currently only designed for three level response
%           and one functional covariate.
%
%   Created: 3/6/2014
%   By: Mark John Meyer
...
model.p                 = p;
model.n                 = n;
G                       = model.V; %% dim of un-transformed X, will be same as p if no additional covariates are included
O                       = size(Y,2); %% dim of Y
model.O                 = O;
...
%         PiMat   = PI*expandop;
    ...
    if   (i > burnin) && (mod(i-burnin,thin) == 0)   %% Save MCMC samples of beta in single matrix.
            ...
            MCMC_beta(is,:)         = reshape(beta',1,G*O); %#zhu#% each row=[beta(1,j,k),j,k|beta(2,j,k),j,k|...|beta(p,j,k),j,k]
            MCMC_cuts(is)           = cuts(2);
...
res.wavespecsx  = wavespecsx;
res.pcaspecsx   = pcaspecsx;
res.MCMCspecs   = MCMCspecs;

%% reshape and store model estimates %%
res.bhat        = reshape(mean(MCMC_beta),G,O);
res.Q025_bhat   = reshape(quantile(MCMC_beta,0.025),G,O);
res.Q975_bhat   = reshape(quantile(MCMC_beta,0.975),G,O);
