function postout = ...
            PostProcess_chains(MCMC_beta,MCMC_alpha,MCMC_flag_theta,MCMC_tau,MCMC_pi,MCMC_theta,theta,model,wavespecsc,wavespecsy,pcaspecsx,twosample,get_sigma,sampleU,MCMC_U)
    %% Outputs:
...
%     MCMC_pi       = res.MCMC_pi;
    ...
    if twosample == 0;
        ...
        postout.Q025_bhat0  = reshape(quantile(Beta0,0.025),Tx,Tx);
        postout.Q975_bhat0  = reshape(quantile(Beta0,0.975),Tx,Tx);
        postout.bhat0       = reshape(mean(Beta0),Tx,Tx);
        postout.Q025_bhat1  = reshape(quantile(Beta1,0.025),Tx,Tx);
        postout.Q975_bhat1  = reshape(quantile(Beta1,0.975),Tx,Tx);
