function res=wavfmm4_v2_zhu(Y,model,wavespecs,MCMCspecs,sampleU) 
%function res=wavfmm4_v2_zhu(D,model,wavespecs,MCMCspecs,sampleU) 
%==========================================================================
% the above function will be reused later on, when dwt is used.
%==========================================================================
%function res=wavfmm4_zhu(True,model,wavespecs,MCMCspecs) 
% Wavefmm4_v2_zhu: Based on wavfmm4_zhu, add IG prior to \tau_{ijk}, and add prior to 
%                  pi_{ij}.
%  
...
if model.Z{1}==0
    ...
    %%% Compute the initial value of theta using MLE and standard deviation.
    model.M=sum(model.m);
    W=GetW(model,D);
...
initial.b_pi=b_pi;
%==========================================================================
PiMat=PI*uneqkron(wavespecs.Kj)'; %#zhu#%  expand from p by J to p by K by repeating within same level.
...
for i=1:(B*thin+burnin)
    ...
    PiMat=PI*uneqkron(wavespecs.Kj)';
    ...
    if (i>burnin)&&(mod(i-burnin,thin)==0)      %% Save MCMC samples of beta in single matrix -- 1 column per iteration
            ...
            MCMC_tau(ii,:)=reshape(tau',1,p*K);   % each block is one i, contains K values. tau_1,...,tau_p.         
...
get_sigma=0;
[betahat,beta_05CI,beta_95CI,alphahat,accept_rate_theta,ghat,ghatns,Q05_ghat,Q95_ghat,MCMC_g,thetahat,theta_05CI,theta_95CI,tauhat,pihat,Sigmat,Sigma,Rho]=...
...
res.betahat=betahat;
res.beta05CI=beta_05CI;
res.beta95CI=beta_95CI;
...
res.ghatns=ghatns;
res.ghat=ghat;
res.Q05_ghat=Q05_ghat;
res.Q95_ghat=Q95_ghat;

res.thetahat=thetahat;
res.MCMC_theta=MCMC_theta;
res.theta_05CI=theta_05CI;
res.theta_95CI=theta_95CI;
